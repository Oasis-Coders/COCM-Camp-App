...
async function signInAs(role: "super_admin" | "admin" | "staff" | "participant", redirectTo?: string) {
  "use server";
  ...
  cookieStore.set("camp-demo-email", `${role}@demo.local`, { httpOnly: true, sameSite: "lax", path: "/" });
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,_#d9edf6_0%,_#fcfcf7_45%,_#f4e8c1_100%)] px-4 py-10">
      <div className="mx-auto max-w-4xl rounded-[32px] bg-white/85 p-8 shadow-panel">
        ...
        <h1 className="mt-4 font-serif text-4xl text-camp-forest">Choose a demo role to explore the shell</h1>
        <p className="mt-4 max-w-2xl text-slate-700">
          This keeps local testing friction low while Supabase project setup happens. The auth callback route
          and server/browser clients are already scaffolded for the real integration.
        </p>

        <div className="mt-6 rounded-2xl border border-camp-forest/10 bg-camp-sand/50 p-4 text-sm text-slate-700">
          Supabase env detected: <span className="font-semibold">{supabaseReady ? "yes" : "no"}</span>
        </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {(["super_admin", "admin", "staff", "participant"] as const).map((role) => (
            <form key={role} action={signInAs.bind(null, role, redirectTo)}>
              ...
              >
                <span className="block font-semibold capitalize text-camp-forest">{role.replace("_", " ")}</span>
