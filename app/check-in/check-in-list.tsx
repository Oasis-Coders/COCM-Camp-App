      ...
      {registrations.length === 0 ? (
        ...
        registrations.map((reg) => {
          ...
          return (
            <article
              key={reg.id}
              className={`flex flex-col justify-between rounded-card border border-camp-forest/10 p-5 shadow-card transition-opacity sm:flex-row sm:items-center ${isPending ? 'opacity-50' : ''} ${reg.isCheckedIn ? 'bg-camp-forest/5' : 'bg-white'}`}
            >
              <div>
                <h3 className="font-semibold text-camp-forest">{name}</h3>
                <p className="mb-2 text-sm text-camp-moss sm:mb-0">{reg.user?.email}</p>
                <div className="mt-1 inline-block rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-camp-moss">
                  {reg.event?.title}
                </div>
              </div>
