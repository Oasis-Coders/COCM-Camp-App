...
async function updateProfile(formData: FormData) {
  ...
  if (!supabase) {
    redirect('/profile?profile=unavailable');
  }
  ...
  if (!input.firstName || !input.lastName) {
    redirect('/profile?profile=missing-fields');
  }
  ...
  if (metadataError) {
    redirect('/profile?profile=metadata-error');
  }
...
async function updateCredentials(formData: FormData) {
  ...
  if (!supabase) {
    redirect('/profile?credentials=unavailable');
  }
  ...
  if (error) {
    redirect('/profile?credentials=error');
  }
...
export async function AccountSettings({ searchParams }: AccountSettingsProps) {
  ...
  const profileMessage = getProfileMessage(searchParams.profile);
  const credentialMessage = getCredentialMessage(searchParams.credentials);
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr,0.9fr]">
        ...
        <article className="rounded-[24px] border border-camp-forest/10 bg-white/85 p-6 shadow-panel">
          ...
          <ul className="mt-4 space-y-3 text-sm text-slate-700">
            ...
            <li>Administrative actions for all users live under Dev Tools.</li>
          </ul>
        </article>
      </div>
